...
- ⚠️ 93 mismatched reviews detected where written text contradicted the star rating
...
## 🔄 Project Workflow
Data Loading → EDA → Text Preprocessing → Sentiment Labelling → TF-IDF Vectorization → Model Training → Evaluation → VADER Comparison → Mismatch Detection → LDA Topic Modelling → Live Predictor

## 🚀 How to Run
Open `majorproject.ipynb` in Google Colab and click **Runtime → Run All**.
